# PQ6 Drone Detector (PySide6) with YOLO, Kalman + OpticalFlow, sound alerts
# Адаптировано для работы на NVIDIA Jetson Orin (Linux ARM64) и Windows
# Использует PySide6 для лучшей совместимости с Jetson
...
import queue
import ctypes
import platform  # Для детекции ОС (Windows/Linux)
from pathlib import Path
import numpy as np
import cv2
from ultralytics import YOLO
from PySide6 import QtCore, QtGui, QtWidgets  # Импорт PySide6 вместо PyQt6
...
def get_short_path(path: str) -> str:
    """Возвращает короткий путь в формате 8.3 для Windows (помогает некоторым сборкам OpenCV). На Linux возвращает оригинал."""
...
def open_video_safely(path: str, log_cb=None, timeout_sec=2.0):
    ...
    Адаптировано для Windows и Linux (Jetson).
    ...
    # Выбор бэкендов в зависимости от ОС
    backends = []
    if platform.system() == "Windows":
...
class WorkerThread(QtCore.QThread):
    frame_ready = QtCore.Signal(np.ndarray)  # RGB ndarray для GUI (PySide6: Signal вместо pyqtSignal)
    status = QtCore.Signal(str)              # Сообщения в лог
    ...
    def run(self):
        ...
        if self.source_spec["type"] == "file":
            ...
            # Адаптация backend для камеры
            apis = []
            if platform.system() == "Windows":
                apis = [cv2.CAP_ANY, getattr(cv2, "CAP_DSHOW", cv2.CAP_ANY), getattr(cv2, "CAP_MSMF", cv2.CAP_ANY)]
            else:  # Linux/Jetson
                apis = [cv2.CAP_ANY, getattr(cv2, "CAP_V4L2", cv2.CAP_ANY)]
            for api in apis:
                try:
...
class PQ6App(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.conf_slider = QtWidgets.QSlider(QtCore.Qt.Horizontal)  # Orientation.Horizontal → Qt.Horizontal
        ...
        self.video_label.setAlignment(QtCore.Qt.AlignCenter)  # AlignmentFlag.AlignCenter → Qt.AlignCenter
    ...
    def start_stop(self):
        ...
        self.worker.frame_ready.connect(self.update_frame, QtCore.Qt.QueuedConnection)  # ConnectionType.QueuedConnection → Qt.QueuedConnection
        self.worker.status.connect(self.log)
        self.worker.start()
        self.btn_start.setText("Stop")
        self.log("[INFO] Worker started")

    @QtCore.Slot(np.ndarray)  # PySide6: @Slot вместо @pyqtSlot
    def update_frame(self, arr: np.ndarray):
        ...
        try:
            ...
            qimg = QtGui.QImage(arr.data, w, h, 3 * w, QtGui.QImage.Format_RGB888)  # Format.Format_RGB888 → Format_RGB888
            pix = QtGui.QPixmap.fromImage(qimg)
            self.video_label.setPixmap(pix.scaled(self.video_label.size(), QtCore.Qt.KeepAspectRatio))  # AspectRatioMode.KeepAspectRatio → Qt.KeepAspectRatio
        except Exception as e:
...
def main():
    ...
    # Установка стиля Fusion на Windows для избежания чёрного окна
    if platform.system() == "Windows":
        app.setStyle("Fusion")
    win = PQ6App()
    win.show()
    sys.exit(app.exec())  # В PySide6: app.exec()
